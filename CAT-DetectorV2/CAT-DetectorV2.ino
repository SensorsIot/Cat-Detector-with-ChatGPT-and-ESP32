...
//char question[] = "Do you recognize a cat on the image. Please answer only with yes or no?";
char question[] = "what do you see on the image?";
...
void withoutTrigger() {
  ...
  client.publish(stateTopicName.c_str(), response.c_str());
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == receiveTopic && message == "occupied") {
    ...
    Serial.println("Analyzing image...");
    String response = sendQuestionToChatGPT("what do you see on the image?", imageBase64);
    // client.publish(stateTopicName.c_str(), response);
    if (isCatDetected(response)) {
      Serial.println("Cat detected!");
      client.publish(stateTopicName.c_str(), "ON");
    } else {
...
bool isCatDetected(const String& analyzeString) {
  ...
  answer.toLowerCase();
  client.publish(stateTopicName.c_str(), answer.c_str());
